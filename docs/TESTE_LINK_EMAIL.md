...
1. Abra o email "Bem-vindo à equipe do VynloBella! 🎉"
...
|-----------------|-----------|
| `https://vynlobella.com/login` | ✅ Link correto no email. Se ao clicar vai para vercel.app, problema é Vercel/DNS |
| `https://resend.com/...` ou `https://resend.io/...` | ⚠️ Resend está usando click tracking. O destino final pode estar errado |
| `https://salon-flow-ten.vercel.app/login` | ❌ Link errado no email. Código não foi atualizado ou há outro lugar gerando o link |
...
- Deve encontrar algo como: `<a href="https://vynlobella.com/login"`
...
3. Veja o que acontece:
   ...
   - ❌ Redireciona para `salon-flow-ten.vercel.app` → Problema no Vercel
...
**Se aparecer:**
- `"loginUrl": "https://vynlobella.com/login"` → ✅ Código está correto
- `"loginUrl": "https://salon-flow-ten.vercel.app/login"` → ❌ Variável de ambiente ainda está errada

---

## Diagnóstico Baseado nos Testes

### Cenário A: Link no email é `vynlobella.com` mas redireciona para `vercel.app`

**Causa:** Configuração do Vercel ou DNS

**Solução:**
1. Vercel Dashboard → Settings → Domains
2. Verifique se `vynlobella.com` está configurado como **Production**
...
### Cenário C: Link no email é `salon-flow-ten.vercel.app`

**Causa:** Código não atualizado ou variável de ambiente errada

**Solução:**
1. Verifique os logs da Edge Function (Teste 4)
2. Se o log mostrar `vynlobella.com`, o problema é no Resend
3. Se o log mostrar `vercel.app`, refaça:
   ```bash
   supabase secrets set SITE_URL=https://vynlobella.com
...
2. **O que acontece** quando acessa `https://vynlobella.com` diretamente?
