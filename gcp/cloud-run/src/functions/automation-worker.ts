/**
 * automation-worker — Cloud Run handler */
  ...
  try {
    ...
    function buildEmailHtml(message: string, clinicName: string): string {
      const escaped = message
        ...
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");
      const lines = escaped.split("\n").map((l: any) => `<p style="margin:0 0 10px 0">${l}</p>`).join("");
    ...
    async function sendEmail(
      ...
      html: string,
      clinicName?: string,
      clinicEmail?: string | null): Promise<{ ok: boolean; details?: string }> {
    ...
    async function dispatch(rule: AutomationRule,
      ...
      if (rule.channel === "whatsapp") {
        ...
        if (!normalized) {
          await db.from("automation_dispatch_logs").insert({
    ...
    async function processAppointmentTrigger(rules: AutomationRule[],
      triggerType: TriggerType,
      sinceMinutes: number,
      publicUrl: string,
      resendKey: string): Promise<DispatchResult> {
      ...
      for (const [tenantId, tenantRules] of byTenant) {
        ...
        const patientIds = [...new Set(appts.map((a: any) => a.patient_id).filter((v: any): v is string => Boolean(v)))];
        const procedureIds = [...new Set(appts.map((a: any) => a.procedure_id).filter((v: any): v is string => Boolean(v)))];
        const profIds = [...new Set(appts.map((a: any) => a.professional_id).filter((v: any): v is string => Boolean(v)))];
        const apptIds = appts.map((a: any) => a.id);

        const [patientsRes, proceduresRes, profsRes, npsRes] = await Promise.all([
          (db.from("patients").select("id, name, phone, email").in("id", patientIds)) as unknown as SelectResult<ClientRow[] | null>,
          procedureIds.length
            ? ((db.from("procedures").select("id, name").in("id", procedureIds)) as unknown as SelectResult<ServiceRow[] | null>)
            : Promise.resolve({ data: [] as ServiceRow[] }),
          profIds.length
            ? ((db.from("profiles").select("id, full_name").in("id", profIds)) as unknown as SelectResult<ProfileRow[] | null>)
            : Promise.resolve({ data: [] as ProfileRow[] }),
          triggerType === "appointment_completed"
            ? ((db.from("nps_responses").select("appointment_id, token").in("appointment_id", apptIds)) as unknown as SelectResult<NpsRow[] | null>)
        ...
        const patientMap = new Map<string, { id: string; name: string | null; phone: string | null; email: string | null }>(
          (patientsRes.data ?? []).map((c: any) => [String(c.id), c]));
        const procedureMap = new Map<string, string>(
          (proceduresRes.data ?? []).map((s: any) => [String(s.id), String(s.name ?? "")]));
    ...
    async function processBirthdayTrigger(rules: AutomationRule[],
      resendKey: string): Promise<DispatchResult> {
      const totals: DispatchResult = { sent: 0, skipped: 0, failed: 0 };
      if (!rules.length) return totals;
      ...
      for (const [tenantId, tenantRules] of byTenant) {
        ...
        const { data: clients, error } = (await db.from("patients")
    ...
    async function processReturnReminderTrigger(rules: AutomationRule[],
      publicUrl: string,
      resendKey: string): Promise<DispatchResult> {
      const totals: DispatchResult = { sent: 0, skipped: 0, failed: 0 };
      if (!rules.length) return totals;
      ...
      for (const [tenantId, tenantRules] of byTenant) {
        ...
        for (const rule of tenantRules) {
          ...
          const { data: returns, error: returnErr } = (await db.from("return_reminders")
          ...
          const clientIds = [...new Set(returns.map((r: any) => r.patient_id))];
          const profIds = [...new Set(returns.map((r: any) => r.professional_id).filter((v: any): v is string => Boolean(v)))];

          const [clientsRes, profsRes] = await Promise.all([
            (db.from("patients").select("id, name, phone, email").in("id", clientIds)) as unknown as SelectResult<ClientRow[] | null>,
            profIds.length
              ? ((db.from("profiles").select("id, full_name").in("id", profIds)) as unknown as SelectResult<ProfileRow[] | null>)
              : Promise.resolve({ data: [] as ProfileRow[] }),
          ]);
          ...
          for (const returnItem of returns) {
            ...
            let confirmLink = "";
            try {
              const { data: tokenData } = await db.rpc("create_return_confirmation_link", {
            ...
            if (r === "sent") {
              ...
              await db.from("return_reminders")
    ...
    async function processInactiveTrigger(rules: AutomationRule[],
      resendKey: string): Promise<DispatchResult> {
      const totals: DispatchResult = { sent: 0, skipped: 0, failed: 0 };
      if (!rules.length) return totals;
      ...
      for (const [tenantId, tenantRules] of byTenant) {
        ...
        for (const rule of tenantRules) {
          ...
          const { data: apptRows, error: apptErr } = (await db.from("appointments")
          ...
          if (candidateClientIds.length) {
            const { data: recentRows } = (await db.from("appointments")
              .select("patient_id")
              .eq("tenant_id", tenantId)
              .neq("status", "cancelled")
            ...
            const recentSet = new Set((recentRows ?? []).map((r: any) => r.patient_id));
            const trueInactiveIds = candidateClientIds.filter((id: any) => !recentSet.has(id));

            if (!trueInactiveIds.length) continue;

            const { data: clients } = (await db.from("patients")
    ...
    async function processQueuedNotifications(rules: AutomationRule[],
      triggerType: string,
      resendKey: string): Promise<DispatchResult> {
      const totals: DispatchResult = { sent: 0, skipped: 0, failed: 0 };
      if (!rules.length) return totals;
      ...
      const { data: queued } = (await db.from("notification_logs")
      ...
      for (const notif of queued) {
        ...
        // Get patient data
        const { data: client } = (await db.from("patients")
          .select("id, name, phone, email")
    ...
    async function processSmartConfirmation(publicUrl: string,
      resendKey: string): Promise<SmartConfirmationResult> {
      ...
      for (const tenant of tenants) {
        const tenantId = (tenant as any).uid;
        ...
        const { data: appts4h } = (await db.from("appointments")
        ...
        for (const appt of appts4h ?? []) {
          ...
          const { data: prof } = appt.professional_id
            ? (await db.from("profiles").select("full_name").eq("id", appt.professional_id).maybeSingle()) as unknown as SelectResult<ProfileRow | null>
            : { data: null };
        ...
        const { data: appts1h } = (await db.from("appointments")
          .select("id, patient_id, procedure_id, professional_id, scheduled_at")
          .eq("tenant_id", tenantId)
          .in("status", ["pending", "confirmed"])
        ...
        const { data: releaseData } = (await db.rpc("auto_release_unconfirmed_appointments")) as unknown as SelectResult<{ released: number } | null>;
    ...
    // ─── Main ──────────────────────────────────────────────────────────────────────
      ...
      const workerKey = process.env.AUTOMATION_WORKER_KEY ?? "";
      ...
      const provided =
        (req.headers['x-automation-worker-key'] as string)?.trim() ||
      ...
      try {
        const { data: allRules, error: rulesErr } = (await db.from("automations")
          .select("*")
        ...
        const byType = (t: TriggerType) => rules.filter((r: any) => r.trigger_type === t);
        ...
        const [created, confirmed, r24h, r2h, completed, birthday, inactive, returnReminder, consentSigned, returnScheduled, smartConfirm] = await Promise.allSettled([
          processAppointmentTrigger(byType("appointment_created"), "appointment_created", sinceMinutes, publicUrl, resendKey),
          processAppointmentTrigger(byType("appointment_confirmed"), "appointment_confirmed", sinceMinutes, publicUrl, resendKey),
          processAppointmentTrigger(byType("appointment_reminder_24h"), "appointment_reminder_24h", sinceMinutes, publicUrl, resendKey),
          processAppointmentTrigger(byType("appointment_reminder_2h"), "appointment_reminder_2h", sinceMinutes, publicUrl, resendKey),
          processAppointmentTrigger(byType("appointment_completed"), "appointment_completed", sinceMinutes, publicUrl, resendKey),
          processBirthdayTrigger(byType("birthday"), resendKey),
          processInactiveTrigger(byType("client_inactive_days"), resendKey),
          processReturnReminderTrigger(byType("return_reminder"), publicUrl, resendKey),
          processQueuedNotifications(byType("consent_signed"), "consent_signed", resendKey),
          processQueuedNotifications(byType("return_scheduled"), "return_scheduled", resendKey),
          processSmartConfirmation(publicUrl, resendKey),
        ]);
        ...
        return res.status(200).json({ success: true, ...totals, smart_confirmation: smartResult });
      } catch (error: any) {
        log("Worker exception", {
          error: error instanceof Error ? error.message : String(error),
          stack: error instanceof Error ? error.stack : undefined,
        });
        return res.status(500).json({ success: false, error: "Erro interno no worker" });
      }

  } catch (err: any) {
    console.error(`[automation-worker] Error:`, err.message || err);
