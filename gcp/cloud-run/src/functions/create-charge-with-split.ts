/**
 * create-charge-with-split — Cloud Run handler */
...
export async function createChargeWithSplit(req: Request, res: Response) {
  try {
    const db = createDbClient();
    const body: ChargeRequest = req.body;

    // Get gateway config
    const { data: gateway, error: gatewayError } = await db.from("tenant_payment_gateways")
      .select("*")
    ...
    if (gatewayError || !gateway) {
      return res.status(400).json({ error: "Gateway de pagamento não configurado", code: "no_gateway" });
    }
    ...
    if (body.professional_id && gateway.is_split_enabled) {
      ...
      const { data: paymentAccount } = await db.from("professional_payment_accounts")
        .select("*")
        .eq("tenant_id", body.tenant_id)
      ...
      if (paymentAccount) {
        // Get commission rule
        const { data: commissionRule } = await db.from("commission_rules")
    ...
    if (splitConfig) {
      ...
      await db.from("split_payment_logs").insert({
    ...
    console.error(`[create-charge-with-split] Error:`, err.message || err);
