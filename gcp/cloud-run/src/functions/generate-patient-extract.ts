/**
 * generate-patient-extract — Cloud Run handler */
...
export async function generatePatientExtract(req: Request, res: Response) {
  try {
    ...
    const authAdmin = createAuthAdmin();
      ...
      if (!tenantId) {
        ...
        const { data: invoices } = await db.from("patient_invoices")
        ...
        const { data: payments } = await (db as any).rpc("get_patient_payment_history_admin", {
    ...
    console.error(`[generate-patient-extract] Error:`, err.message || err);
