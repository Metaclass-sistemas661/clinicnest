/**
 * invite-team-member — Cloud Run handler */
    ...
    // CORS handled by middleware
      ...
      // DB accessed via shared/db module
        ...
        if (role !== "staff" && role !== "admin") {
          return res.status(400).json({ error: "Função deve ser staff ou admin" });
        }

        log("Tenant encontrado", { tenant_id: tenantId });

        log("Verificando se usuário é admin");
        const { data: roleRow, error: roleError } = await db.from("user_roles")
          .select("role")
          .eq("user_id", user.id)
        ...
        if (roleError) {
          ...
          return res.status(500).json({ error: `Erro ao verificar permissões: ${roleError.message}` });
        }
        ...
        try {
          const { data: subRow, error: subErr } = await db.from("subscriptions")
            .select("plan")
            .eq("tenant_id", tenantId)
            .maybeSingle();
          ...
          const { count: totalProfiles, error: countErr } = await db.from("profiles")
            .select("id")
            .eq("tenant_id", tenantId);
          ...
          if (effectiveUsers >= additionalLimit) {
            ...
            return res.status(403).json({
                ...
                details: {
                  ...
                  required_tier: tier === "starter" ? "solo" : "premium",
                },
              });
          }
        } catch (limitErr: any) {
        ...
        if (createError) {
          ...
          log("ERROR: Erro ao criar usuário", { error: createError.message, code: createError.code });
        ...
        log("Usuário criado com sucesso", { user_id: newUser?.user?.uid });
        ...
        await auditLog({ tenantId,
          ...
          entityType: "user",
          entityId: newUser?.user?.uid ?? null,
          metadata: {
            invitedRole: role,
            emailSent,
          },
