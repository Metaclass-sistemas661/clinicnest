/**
 * notify-patient-appointment — Cloud Run handler */
...
export async function notifyPatientAppointment(req: Request, res: Response) {
  try {
    const payload: AppointmentNotificationPayload = req.body;
        const { appointment_id, notification_type } = payload;

        if (!appointment_id || !notification_type) {
          return res.status(400).json({ error: "appointment_id e notification_type são obrigatórios" });
        }
        ...
        const [clientResult, professionalResult, serviceResult, tenantResult] = await Promise.all([
          db.from("patients").select("id, name, email, phone").eq("id", appointment.patient_id).single(),
          db.from("profiles").select("id, full_name").eq("user_id", appointment.professional_id).single(),
          db.from("procedures").select("id, name, duration_minutes").eq("id", appointment.procedure_id).single(),
          db.from("tenants").select("id, name, phone, address").eq("id", appointment.tenant_id).single(),
        ]);
        ...
        const [emailSent, pushSent] = await Promise.all([
          ...
          clientResult.data.uid
            ? sendPushNotification(clientResult.data.id, pushMessage.title, pushMessage.body, {
                type: "appointment",
                appointment_id,
                notification_type,
              })
    ...
    console.error(`[notify-patient-appointment] Error:`, err.message || err);
