...
async function getAccessToken(tenantId: string,
  ...
  const { data: existingToken } = await db.from('rnds_tokens')
  ...
  const { data: certificate } = await db.from('rnds_certificates')
  ...
  try {
    ...
    await db.from('rnds_tokens').insert({
      tenant_id: tenantId,
...
async function submitToRNDS(
  ...
  try {
    ...
    const errorCode = (responseData as any)?.issue?.[0]?.code || response.status.toString();
    const errorMessage = (responseData as any)?.issue?.[0]?.diagnostics ||
    ...
    const {} = {
      ...
      try {
        ...
        for (const submission of submissions as Submission[]) {
          ...
          const accessToken = await getAccessToken(submission.tenant_id,
            submission.rnds_cnes,
            submission.rnds_environment
          );

          if (!accessToken) {
            await db.rpc('update_rnds_submission_status', {
