/**
 * seal-consent-pdf — Cloud Run handler */
...
import { createStorageClient } from '../shared/gcs';
import { PDFDocument, StandardFonts, rgb } from 'pdf-lib' ;
export async function sealConsentPdf(req: Request, res: Response) {
  try {
    ...
    // ─── Main ──────────────────────────────────────────────────────────────────────
      try {
        const { consent_id } = req.body as { consent_id?: string };

        if (!consent_id) {
          return res.status(400).json({ error: "consent_id é obrigatório" });
        }
        ...
        const authRes = (await authAdmin.getUser(token) as any);

        const authErr = authRes.error;

        const user = authRes.data?.user;
        if (authErr || !user) {
          log("Invalid JWT", { error: authErr?.message });
          return res.status(401).json({ error: "Invalid or expired token" });
        }

        // ── 1. Buscar consent com dados relacionados ──
        const { data: consent, error: consentErr } = await db.from("patient_consents")
          .select("*")
          .eq("id", consent_id)
          .single();
        ...
        if (consentErr || !consent) {
          log("Consent not found", { consent_id });
        ...
        const isOwner = consent.patient_user_id === user.uid;
        const userRole = user.app_metadata?.role ?? user.user_metadata?.account_type;
        const isStaff = userRole !== "patient";
        if (!isOwner && !isStaff) {
          log("Forbidden: user is not consent owner or staff", { userId: user.id, consentOwner: consent.patient_user_id });
          return res.status(403).json({ error: "Sem permissão para selar este consentimento" });
        }

        if (!consent.signed_at) {
          return res.status(422).json({ error: "Consentimento ainda não foi assinado" });
        }
        ...
        const { data: template } = await db.from("consent_templates")
          .select("title, slug")
        ...
        if (consent.signature_method === "manual" && consent.manual_signature_path) {
          try {
            const { data: sigData, error: sigDlErr } = await storageClient.from("consent-signatures")
            ...
            if (sigData) {
              const sigBytes = new Uint8Array(sigData as any);
              let sigImage;
              try {
            ...
            log("Could not embed signature image", err);
          }
        }
        ...
        if (consent.facial_photo_path) {
          try {
            const { data: photoData, error: photoDlErr } = await storageClient.from("consent-photos")
              .download(consent.facial_photo_path);
            if (photoDlErr) console.warn("[seal-consent-pdf] Photo download error:", photoDlErr.message);

            if (photoData) {
              const photoBytes = new Uint8Array(photoData as any);
              let photoImage;
            ...
            log("Could not embed facial photo", err);
          }
        }
        ...
        const { error: uploadErr } = await storageClient.from("consent-sealed-pdfs")
        ...
        if (uploadErr) {
          log("Upload failed", uploadErr);
          return res.status(500).json({ error: "Falha ao fazer upload do PDF" });
        }

        // ── 8. Registrar no banco via RPC ──
        const { data: sealResult, error: sealErr } = await db.rpc("seal_consent_pdf", {
        ...
        if (sealErr) {
          log("seal_consent_pdf RPC failed", sealErr);
          return res.status(500).json({ error: "Falha ao registrar selagem no banco" });
        }

        log("Consent PDF sealed successfully", { consent_id, hash: pdfHash });
    ...
    console.error(`[seal-consent-pdf] Error:`, err.message || err);
