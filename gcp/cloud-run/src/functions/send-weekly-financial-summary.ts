/**
 * send-weekly-financial-summary — Cloud Run handler */
...
export async function sendWeeklyFinancialSummary(req: Request, res: Response) {
  try {
    const db = createDbClient();
      try {
        ...
        const { data: preferences, error: prefError } = await db.from("user_notification_preferences")
          .select("user_id")
        ...
        for (const profile of profiles || []) {
          ...
          const { data: commissionsGenerated } = await db.from("commission_payments")
            .select("amount")
          ...
          const totalGenerated = (commissionsGenerated || []).reduce(
            (sum: any, c: any) => sum + Number(c.amount || 0),
            0
          );

          // Comissões pagas na semana
          const { data: commissionsPaid } = await db.from("commission_payments")
            .select("amount")
            .eq("tenant_id", profile.tenant_id)
            .eq("professional_id", profile.user_id)
            .eq("status", "paid")
          ...
          const { data: salariesPaid } = await db.from("salary_payments")
            .select("amount")
            .eq("tenant_id", profile.tenant_id)
            .eq("professional_id", profile.user_id)
            .eq("status", "paid")
          ...
          const totalSalPaid = (salariesPaid || []).reduce(
            (sum: any, s: any) => sum + Number(s.amount || 0),
            0
          );

          // Total pendente
          const { data: pendingCommissions } = await db.from("commission_payments")
            .select("amount")
            .eq("tenant_id", profile.tenant_id)
            .eq("professional_id", profile.user_id)
            .eq("status", "pending");
          ...
          const { count: appointmentsCount } = await db.from("appointments")
            .select("id")
            .eq("tenant_id", profile.tenant_id)
            .eq("professional_id", profile.user_id)
            .eq("status", "completed")
        ...
        for (const summary of summaries) {
          ...
          await db.from("notification_logs").insert({
            tenant_id: profiles?.find((p: any) => p.user_id === summary.professional_id)?.tenant_id,
    ...
    console.error(`[send-weekly-financial-summary] Error:`, err.message || err);
