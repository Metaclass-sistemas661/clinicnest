/**
 * submit-contact-message — Cloud Run handler */
  ...
  try {
    const db = createDbClient();
        ...
        if (channel === "lgpd" && !phone) {
          return res.status(400).json({ success: false, error: "Preencha nome, celular, e-mail e mensagem." });
        }
        ...
        const { data: createdMessage, error: insertError } = await db.from("contact_messages")
          .insert({
            name,
            email,
        ...
        if (insertError) {
          ...
          return res.status(500).json({ success: false, error: "Erro ao registrar mensagem." });
        }

        const adminEmail = (process.env.CONTACT_ADMIN_EMAIL ?? "contato@metaclass.com.br").trim();
        const emailFrom = (process.env.CONTACT_EMAIL_FROM ?? "ClinicNest <no-reply@metaclass.com.br>")
          .trim();
    ...
    console.error(`[submit-contact-message] Error:`, err.message || err);
