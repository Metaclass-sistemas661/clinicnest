/**
 * validate-council-number — Cloud Run handler */

import { Request, Response } from 'express';

export async function validateCouncilNumber(req: Request, res: Response) {
  try {
    ...
    async function validateCRM(number: string, state: string): Promise<{
      ...
      try {
        ...
        if (!council_type || !council_number || !council_state) {
          return res.status(400).json({ valid: false, message: "council_type, council_number e council_state são obrigatórios." });
        }
        ...
        if (!VALID_UFS.includes(state)) {
          return res.status(200).json({ valid: false, message: "UF inválida." });
        }

        // Validar formato
        const formatResult = validateFormat(type, number);
        if (!formatResult.valid) {
          return res.status(200).json({ valid: false, message: formatResult.message });
        }
        ...
        return res.status(500).json({ valid: false, message: "Erro interno ao validar conselho." });
      }

  } catch (err: any) {
    console.error(`[validate-council-number] Error:`, err.message || err);
