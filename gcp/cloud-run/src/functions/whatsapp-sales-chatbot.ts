/**
 * whatsapp-sales-chatbot — Cloud Run handler */
...
// ─── Config ──────────────────────────────────────────────────────────────────
const SALES_CHATBOT_SECRET = process.env.SALES_CHATBOT_SECRET || process.env.CRON_SECRET || "";
const EVOLUTION_API_URL = process.env.EVOLUTION_SALES_API_URL || process.env.EVOLUTION_API_URL || "";
const EVOLUTION_API_KEY = process.env.EVOLUTION_SALES_API_KEY || process.env.EVOLUTION_API_KEY || "";
const EVOLUTION_INSTANCE = process.env.EVOLUTION_SALES_INSTANCE || "clinicnest-vendas";
...
async function getOrCreateConversation(phone: string): Promise<SalesConversation> {
  const { data: existing } = await db.from("sales_chatbot_conversations")
    .select("*")
    .eq("phone", phone)
    .maybeSingle();
  ...
  if (existing) {
    ...
    if (diffMin > SESSION_TIMEOUT_MIN) {
      await db.from("sales_chatbot_conversations")
        .update({
      ...
      await db.from("sales_chatbot_messages")
        .delete()
...
async function updateState(conversationId: string,
  newState: State,
...
async function saveMessage(conversationId: string,
  direction: "inbound" | "outbound",
  content: string) {
  await db.from("sales_chatbot_messages").insert({
...
async function getAiHistory(conversationId: string): Promise<any[]> {
  const { data: messages } = await db.from("sales_chatbot_messages")
...
async function updateLeadData(conversationId: string,
  phone: string,
  ...
  await db.from("sales_chatbot_conversations").update(updates).eq("id", conversationId);
  ...
  const { data: existingLead } = await db.from("sales_leads")
    .select("id")
    .eq("phone", phone)
    .maybeSingle();

  if (existingLead) {
    await db.from("sales_leads").update(leadUpsert).eq("id", existingLead.id);
  } else {
    await db.from("sales_leads").insert(leadUpsert);
  }
...
async function handleIncoming(phone: string,
  message: string,
  _pushName?: string): Promise<string> {
  ...
  if (inputLower === "menu" || inputLower === "voltar" || inputLower === "inicio") {
    await updateState(conversation.id, STATE.MAIN_MENU);
    return MSG.WELCOME;
  }
  if (inputLower === "0" || inputLower === "sair" || inputLower === "encerrar" || inputLower === "finalizar" || inputLower === "tchau") {
    await updateState(conversation.id, STATE.ENDED);
    return MSG.GOODBYE;
  }
  ...
  switch (currentState) {
    ...
    case STATE.MAIN_MENU: {
      switch (input) {
        case "1":
          await updateState(conversation.id, STATE.FEATURES_MENU);
          return MSG.FEATURES + MSG.MENU_BACK;
        case "2":
          await updateState(conversation.id, STATE.PLANS_MENU);
          return MSG.PLANS + MSG.MENU_BACK;
        case "3":
          await updateState(conversation.id, STATE.FREE_TRIAL);
          return MSG.FREE_TRIAL + MSG.MENU_BACK;
        case "4":
          await db.from("sales_chatbot_conversations")
            .update({ is_human_takeover: true, updated_at: new Date().toISOString() })
            .eq("id", conversation.id);
          await updateState(conversation.id, STATE.HUMAN_TAKEOVER);
          return MSG.HUMAN_TRANSFER;
        case "5":
          await updateState(conversation.id, STATE.AI_QUESTION);
          return MSG.AI_INTRO;
        default:
          return MSG.INVALID;
      }
    ...
    case STATE.FREE_TRIAL: {
      ...
      await updateState(conversation.id, STATE.COLLECTING_NAME);
      return MSG.ASK_NAME;
    }

    case STATE.COLLECTING_NAME: {
      const name = input.split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(" ");
      await updateLeadData(conversation.id, phone, { name });
      await updateState(conversation.id, STATE.COLLECTING_EMAIL, { leadName: name });
    ...
    case STATE.COLLECTING_EMAIL: {
      ...
      if (input.includes("@") && input.includes(".")) {
        await updateLeadData(conversation.id, phone, { email: input.toLowerCase() });
        await updateState(conversation.id, STATE.COLLECTING_SIZE, { leadName: name, leadEmail: input.toLowerCase() });
        return MSG.ASK_CLINIC_SIZE(name);
      }
      // If not email, maybe they skipped — ask clinic size
      await updateState(conversation.id, STATE.COLLECTING_SIZE, { leadName: name });
      return MSG.ASK_CLINIC_SIZE(name);
    }
    ...
    case STATE.COLLECTING_SIZE: {
      ...
      if (sizeNum > 0) {
        await updateLeadData(conversation.id, phone, { professionals: sizeNum });
      }
      await updateState(conversation.id, STATE.MAIN_MENU);
    ...
    case STATE.AI_QUESTION: {
      ...
      try {
        const history = await getAiHistory(conversation.id);
        let messages: any[];
        if (history.length <= 1) {
        ...
        const result = await chatCompletion((messages as any).map((m: any) => ({ role: m.role === "assistant" ? "model" as const : "user" as const, text: m.content })), { systemInstruction: AI_PROMPT,
          maxTokens: 300,
          temperature: 0.5,
        });

        // Clean any leaked tags
        const cleaned = String(result.text).replace(/\s*\[LEAD:[^\]]*\]\s*/g, "")
          .trim();
...
export async function whatsappSalesChatbot(req: Request, res: Response) {
  try {
      try {
        ...
        const headerApiKey = (req.headers['apikey'] as string) || (req.headers['x-api-key'] as string) || "";

        const body: IncomingWebhook = req.body;
        ...
        const reply = await handleIncoming(phone, incomingMessage, pushName);

        if (reply) {
          // Save outbound message (inbound is saved inside handleIncoming)
          const { data: conv } = await db.from("sales_chatbot_conversations")
            .select("id")
            .eq("phone", phone)
            .maybeSingle();
          if (conv) {
            await saveMessage(conv.id, "outbound", reply);
          }
