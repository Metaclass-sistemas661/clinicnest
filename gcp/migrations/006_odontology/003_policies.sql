-- ============================================================
-- GCP Cloud SQL Migration - 005_policies_odontology.sql
-- Execution Order: 015
...
CREATE POLICY "odontogram_annotations_insert" ON public.odontogram_annotations
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE o.id = odontogram_id
        AND o.tenant_id = public.get_user_tenant_id(current_setting('app.current_user_id')::uuid)
        AND (public.is_tenant_admin(current_setting('app.current_user_id')::uuid, o.tenant_id) OR public.is_dentist(current_setting('app.current_user_id')::uuid))
    )
  );
...
CREATE POLICY "odontogram_annotations_delete" ON public.odontogram_annotations
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.odontograms o
      WHERE o.id = odontogram_id
        AND public.is_tenant_admin(current_setting('app.current_user_id')::uuid, o.tenant_id)
    )
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE o.id = odontogram_id
        AND o.tenant_id = public.get_user_tenant_id(current_setting('app.current_user_id')::uuid)
        AND (
          public.is_tenant_admin(current_setting('app.current_user_id')::uuid, o.tenant_id)
...
CREATE POLICY "periogram_measurements_access" ON periogram_measurements
  FOR ALL USING (
    periogram_id IN (
      SELECT id FROM periograms WHERE tenant_id = (SELECT tenant_id FROM profiles WHERE id = current_setting('app.current_user_id')::uuid)
    )
  );
...
CREATE POLICY "treatment_plan_items_select" ON public.treatment_plan_items
  ...
  USING (
    EXISTS (
      ...
      WHERE p.id = plan_id
        AND p.tenant_id = public.get_user_tenant_id(current_setting('app.current_user_id')::uuid)
    )
  );
...
CREATE POLICY "treatment_plan_items_insert" ON public.treatment_plan_items
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE p.id = plan_id
        AND p.tenant_id = public.get_user_tenant_id(current_setting('app.current_user_id')::uuid)
        AND (public.is_tenant_admin(current_setting('app.current_user_id')::uuid, p.tenant_id) OR public.is_dentist(current_setting('app.current_user_id')::uuid))
    )
  );
...
CREATE POLICY "treatment_plan_items_delete" ON public.treatment_plan_items
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.treatment_plans p
      WHERE p.id = plan_id
        AND public.is_tenant_admin(current_setting('app.current_user_id')::uuid, p.tenant_id)
    )
  );
