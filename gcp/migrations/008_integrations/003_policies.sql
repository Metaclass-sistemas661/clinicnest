-- ============================================================
-- GCP Cloud SQL Migration - 005_policies_integrations.sql
-- Execution Order: 015
...
CREATE POLICY "Tenant can view own verifications" ON public.document_verifications
  ...
  USING (
    tenant_id IN (
      SELECT tenant_id FROM public.profiles WHERE id = current_setting('app.current_user_id')::uuid
    )
  );
...
CREATE POLICY rnds_certificates_tenant_isolation ON rnds_certificates
  FOR ALL USING (tenant_id = get_user_tenant_id(current_setting('app.current_user_id')::uuid));
