CREATE OR REPLACE FUNCTION public.verify_document_public(p_hash text, p_verifier_ip inet DEFAULT NULL::inet, p_verifier_user_agent text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- 1. Buscar em documentos m├®dicos (view existente)
  ...
  IF v_document IS NOT NULL THEN
    ...
    v_result := jsonb_build_object(
      ...
      'message', CASE
        ...
        WHEN v_is_valid THEN 'Documento v├ílido e assinado digitalmente'

        ELSE 'Documento encontrado mas n├úo assinado'
  ...
  ELSE
    ...
    IF v_consent IS NOT NULL THEN
      ...
      v_result := jsonb_build_object(
        ...
        'message', CASE
          ...
          WHEN v_is_valid THEN 'Termo de consentimento v├ílido ÔÇö PDF selado digitalmente'

          ELSE 'Termo encontrado mas PDF ainda n├úo foi selado'
    ...
    ELSE
      ...
      v_result := jsonb_build_object(
        ...
        'message', 'Documento n├úo encontrado ou n├úo assinado digitalmente'
  ...
  -- Registrar verifica├º├úo no log
