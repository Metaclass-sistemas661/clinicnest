CREATE OR REPLACE FUNCTION public.finalize_order_v1(p_order_id uuid, p_payments jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_order.status NOT IN ('draft', 'open') THEN
    ...
    RAISE EXCEPTION 'Comanda j├í finalizada ou cancelada (status: %)', v_order.status USING DETAIL = 'VALIDATION_ERROR';
  ...
  LOOP
    ...
    IF v_pm_id IS NULL OR v_pm_amount IS NULL OR v_pm_amount <= 0 THEN
      ...
      RAISE EXCEPTION 'Pagamento inv├ílido' USING DETAIL = 'VALIDATION_ERROR';
    ...
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'M├®todo de pagamento inv├ílido' USING DETAIL = 'VALIDATION_ERROR';
  ...
  -- ÔöÇÔöÇÔöÇ STOCK VALIDATION (pre-check all products before any mutation) ÔöÇÔöÇÔöÇ
  ...
  LOOP
    ...
    IF v_item.stock_qty < v_item.quantity THEN
      ...
      RAISE EXCEPTION 'Estoque insuficiente para "%": dispon├¡vel %, necess├írio %',
  ...
  -- ÔöÇÔöÇÔöÇ CREATE PAYMENTS ÔöÇÔöÇÔöÇ
  ...
  -- ÔöÇÔöÇÔöÇ STOCK MOVEMENTS (out, sale) for product items + CMV snapshot ÔöÇÔöÇÔöÇ
  ...
  LOOP
    ...
    -- Snapshot de custo m├®dio no momento da venda
  ...
  -- ÔöÇÔöÇÔöÇ FINANCIAL TRANSACTIONS (income) ÔöÇÔöÇÔöÇ
  ...
  IF EXISTS (SELECT 1 FROM public.order_items WHERE order_id = p_order_id AND kind = 'service') THEN
    ...
    INSERT INTO public.financial_transactions (
      ...
      'Comanda #' || left(p_order_id::text, 8) || ' ÔÇö servi├ºos',
  ...
  IF EXISTS (SELECT 1 FROM public.order_items WHERE order_id = p_order_id AND kind = 'product') THEN
    ...
    INSERT INTO public.financial_transactions (
      ...
      'Comanda #' || left(p_order_id::text, 8) || ' ÔÇö produtos',
  ...
  -- ÔöÇÔöÇÔöÇ UPDATE ORDER STATUS ÔöÇÔöÇÔöÇ
  ...
  -- ÔöÇÔöÇÔöÇ UPDATE APPOINTMENT STATUS to completed ÔöÇÔöÇÔöÇ
