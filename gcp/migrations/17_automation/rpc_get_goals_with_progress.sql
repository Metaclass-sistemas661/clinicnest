CREATE OR REPLACE FUNCTION public.get_goals_with_progress(p_tenant_id uuid, p_include_archived boolean DEFAULT false)
 RETURNS TABLE(id uuid, name text, goal_type text, target_value numeric, period text, professional_id uuid, product_id uuid, show_in_header boolean, header_priority integer, custom_start date, custom_end date, archived_at timestamp with time zone, current_value numeric, progress_pct numeric, days_remaining integer, period_elapsed_pct numeric, projected_reach text, period_start timestamp with time zone, period_end timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
