-- ============================================================
-- GCP Cloud SQL Migration - 005_policies_clinical.sql
-- Execution Order: 015
...
CREATE POLICY clinical_evolutions_delete ON public.clinical_evolutions
  ...
  USING (
    ...
    AND professional_id IN (SELECT id FROM public.profiles WHERE user_id = current_setting('app.current_user_id')::uuid)
  );
...
CREATE POLICY "medical_certificates_insert" ON public.medical_certificates
  ...
  WITH CHECK (
    ...
    AND (
      public.is_tenant_admin(current_setting('app.current_user_id')::uuid, tenant_id)
      OR public.is_prescriber(current_setting('app.current_user_id')::uuid)
    )
  );
...
CREATE POLICY "medical_certificates_update" ON public.medical_certificates
  ...
  USING (
    ...
    AND (
      public.is_tenant_admin(current_setting('app.current_user_id')::uuid, tenant_id)
      OR (
        public.is_prescriber(current_setting('app.current_user_id')::uuid)
        AND public.has_treated_patient(current_setting('app.current_user_id')::uuid, patient_id)
      )
    )
  )
    ...
    AND (
      ...
      OR (
        EXISTS (
          ...
          WHERE user_id = current_setting('app.current_user_id')::uuid AND professional_type = 'enfermeiro'
        )
      )
    )
  );
...
CREATE POLICY pre_consultation_forms_tenant ON pre_consultation_forms
  FOR ALL USING (tenant_id = public.get_user_tenant_id(current_setting('app.current_user_id')::uuid));
...
CREATE POLICY "profile_certificates_select" ON public.profile_certificates
  FOR SELECT TO authenticated
  USING (
    profile_id = (SELECT id FROM public.profiles WHERE user_id = current_setting('app.current_user_id')::uuid)
    OR tenant_id = public.get_user_tenant_id(current_setting('app.current_user_id')::uuid)
  );
...
CREATE POLICY "return_confirmation_tokens_tenant_access" ON return_confirmation_tokens
  FOR ALL
  USING (tenant_id = get_user_tenant_id(current_setting('app.current_user_id')::uuid));
...
CREATE POLICY "triage_records_insert" ON public.triage_records
  ...
  WITH CHECK (
    ...
    AND (
      public.is_tenant_admin(current_setting('app.current_user_id')::uuid, tenant_id)
      OR public.is_nursing_professional(current_setting('app.current_user_id')::uuid)
    )
  );
