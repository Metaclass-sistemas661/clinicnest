    ...
    <Card className="border-gradient">
      <CardContent className="p-4">
        ...
        {loading ? (
          ...
          <p className={`text-xl font-bold ${colors[color]}`}>{formatCurrency(value)}</p>
        )}
...
export default function FluxoDeCaixa() {
  ...
  const fetchProjection = useCallback(async () => {
    ...
    try {
      ...
      logger.error("FluxoDeCaixa.fetch", err);
  ...
  // Build chart data: group by every few days for readability when window is large
  ...
  const upcomingPayable = useMemo(() => {
    if (!projection?.series) return [];
    return projection.series
      .filter((p) => !p.is_past && p.projected_payable > 0)
      .slice(0, 10);
  }, [projection]);
  ...
  const upcomingReceivable = useMemo(() => {
    if (!projection?.series) return [];
    return projection.series
      .filter((p) => !p.is_past && p.projected_receivable > 0)
      .slice(0, 10);
  }, [projection]);

  const projectedBalance = useMemo(() => {
    if (!projection) return 0;
    return (projection.opening_balance ?? 0)
      + (projection.projected_receivable_window ?? 0)
      - (projection.projected_payable_window ?? 0);
  }, [projection]);
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Visão projetada das entradas e saídas futuras</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <StatCard
            label="Saldo Atual"
            value={projection?.opening_balance ?? 0}
            sub="Com base nas transações"
            icon={Wallet}
            color="purple"
            loading={isLoading}
          />
          <StatCard
            label={`A Pagar (${days}d)`}
            value={projection?.projected_payable_window ?? 0}
          ...
          />
          <StatCard
            label={`A Receber (${days}d)`}
            value={projection?.projected_receivable_window ?? 0}
            sub="Contas a receber pendentes"
            icon={ArrowUpCircle}
            color="green"
            loading={isLoading}
          />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card className="border-gradient">
            ...
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <p className="text-center py-8 text-sm text-muted-foreground">Nenhuma saída prevista</p>
              ) : (
                <Table>
                  <TableHeader>
                    ...
                    {upcomingPayable.map((p) => (
                      <TableRow key={p.date}>
                        ...
                        <TableCell className="text-right font-semibold text-destructive text-sm">
                          -{formatCurrency(p.projected_payable)}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
          ...
          <Card className="border-gradient">
            ...
            <CardContent className="p-0">
              ...
              ) : upcomingReceivable.length === 0 ? (
                <p className="text-center py-8 text-sm text-muted-foreground">Nenhuma entrada prevista</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/20">
                    ...
                    {upcomingReceivable.map((p) => (
                      <TableRow key={p.date}>
                        ...
                        <TableCell className="text-right font-semibold text-emerald-600 text-sm">
                          +{formatCurrency(p.projected_receivable)}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
