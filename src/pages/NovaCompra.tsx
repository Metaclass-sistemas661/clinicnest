...
import { createPurchaseV1 } from "@/lib/typed-rpc";
    ...
    const load = async () => {
      ...
      try {
        const [supRes, prodRes] = await Promise.all([
          api.from("suppliers").select("id,name").eq("tenant_id", profile.tenant_id).order("name").limit(200),
          api.from("products").select("id,name,cost,quantity").eq("tenant_id", profile.tenant_id).eq("is_active", true).order("name").limit(500),
        ]);
