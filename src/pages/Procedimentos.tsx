...
export default function Procedimentos() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await upsertProcedureV2({
        p_procedure_id: editingProcedure?.id ?? null,
      ...
      if (error) {
        toastRpcError(toast, error as any, editingProcedure ? "Erro ao atualizar procedimento" : "Erro ao cadastrar procedimento");
        return;
      }

      toast.success(editingProcedure ? "Procedimento atualizado com sucesso!" : "Procedimento cadastrado com sucesso!");
      ...
      toast.error(editingProcedure ? "Erro ao atualizar procedimento" : "Erro ao cadastrar procedimento");
      logger.error(error);
    } finally {
      setIsSaving(false);
    }
  ...
  const toggleActive = async (procedure: Procedure) => {
    try {
      const { error } = await setProcedureActiveV2({
        p_procedure_id: procedure.id,
        p_is_active: !procedure.is_active,
      });
      ...
      toast.success(procedure.is_active ? "Procedimento desativado" : "Procedimento ativado");
      fetchProcedures();
    } catch (_error) {
        ...
        isAdmin ? (
          <>
            ...
            <FormDrawer
              ...
              submitLabel={editingProcedure ? "Atualizar" : "Cadastrar"}
            >
        ...
        <CardContent>
          {isLoading ? (
            ...
            <>
              ...
              <div className="block md:hidden space-y-3">
                {procedures.map((procedure) => (
                  <div
                    key={procedure.id}
                    className={`rounded-lg border p-4 space-y-3 ${!procedure.is_active ? "opacity-60" : ""}`}
                  >
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-semibold">{formatCurrency(procedure.price)}</span>
                    </div>
                    {isAdmin && (
                    <div className="flex items-center gap-2 pt-2 border-t">
                      <Button variant="ghost" size="sm" onClick={() => handleOpenDialog(procedure)} aria-label={`Editar procedimento ${procedure.name}`} data-tour="procedures-item-edit">
                        <Pencil className="h-4 w-4" />
                      </Button>
                      <Switch
                        checked={procedure.is_active}
                        onCheckedChange={() => toggleActive(procedure)}
                        data-tour="procedures-item-toggle-active"
                      />
            ...
            <Table>
              ...
              <TableBody>
                {procedures.map((procedure) => (
                  <TableRow key={procedure.id} className={!procedure.is_active ? "opacity-60" : ""}>
                    <TableCell>
                      <div>
                        <p className="font-medium">{procedure.name}</p>
                        {procedure.description && (
                    ...
                    <TableCell className="text-right">
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <Button
                          ...
                          data-tour="procedures-item-edit"
                        >
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Switch
