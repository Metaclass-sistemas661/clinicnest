...
async function collectMetrics(
  ...
  // New patients
  const { count: newPatients } = await sb
    .from("patients")
  ...
  return {
    ...
    totalRevenue,
    totalProcedures: topProcedures.reduce((s, p) => s + p.count, 0),
    topProcedures,
    topProfessionals,
...
- Top 5 procedimentos: ${metrics.topProcedures.map((p) => `${p.name} (${p.count})`).join(", ") || "N/A"}
- Top 5 profissionais (por volume): ${metrics.topProfessionals.map((p) => `${p.name} (${p.count})`).join(", ") || "N/A"}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    for (const tenant of tenants) {
      try {
        // Collect metrics
        const metrics = await collectMetrics(sb, tenant.id, period);
        ...
        for (const admin of recipients) {
          const result = await sendClinicEmail({
            to: admin.email,
            subject: `📊 Resumo Semanal — ${tenant.name ?? "Sua Clínica"}`,
            clinic,
            headline: "Resumo Semanal",
            icon: "📊",
            bodyHtml: emailBody,
            bodyText: plainText,
          });
