...
serve(async (req) => {
  ...
  try {
    ...
    if (!patientId) {
      return new Response(
        JSON.stringify({ error: "patient_id é obrigatório" }),
        { status: 400, headers: { ...cors, "Content-Type": "application/json" } },
      );
    }

    // ── Authorization: admin do tenant OU o próprio paciente ──
    const { data: adminCheck } = await supabase.rpc("is_tenant_admin", {
      p_user_id: user.id,
    ...
    const { data: patientSelf } = await supabase
      .from("patients")
      .select("id")
      .eq("id", patientId)
    ...
    if (!isAdmin && !patientSelf) {
      return new Response(
        JSON.stringify({ error: "Acesso negado. Somente admin ou o próprio paciente." }),
    ...
    const bundle = {
      ...
      entry: resources.map(r => ({
        fullUrl: `urn:uuid:${r.id ?? fhirId()}`,
        resource: r,
      })),
    };

    // ── Audit log ──
    await supabase.rpc("log_tenant_action", {
      p_tenant_id: tenantId,
    ...
    return new Response(
      JSON.stringify(bundle),
      {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="fhir-patient-${patientId}.json"`,
        },
      },
    );
  } catch (error) {
