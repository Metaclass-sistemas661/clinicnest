-- Corrige cálculo de comissão: usa APENAS config do profissional (Equipe) ou valor explícito no agendamento.
-- Remove o fallback de default_commission_percent (Configurações). Sem config em Equipe = não cria comissão.
    ...
    IF v_professional_user_id IS NOT NULL THEN
      IF NOT EXISTS (
        ...
        ELSIF v_commission_config IS NOT NULL THEN
          ...
          END IF;
        END IF;
        -- Sem config em Equipe: NÃO usa default do tenant, NÃO cria comissão
