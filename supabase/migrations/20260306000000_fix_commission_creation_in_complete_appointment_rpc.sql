-- Fix: ensure commission creation uses tenant from appointment and provides deterministic lookup + diagnostics
...
BEGIN
  ...
  -- Buscar agendamento primeiro e usar tenant do pr├│prio agendamento (fonte de verdade)
  SELECT
    a.*,
  ...
  -- Validar perfil do requisitante no tenant do agendamento
  SELECT p.id
  ...
  IF v_appointment.status = 'completed' THEN
    RAISE EXCEPTION 'Agendamento j├í foi conclu├¡do';
  END IF;
  ...
  IF p_product_id IS NOT NULL THEN
    ...
    INSERT INTO public.stock_movements (
      ...
      'out',
      'sale',
      COALESCE('Venda durante o servi├ºo ' || v_appointment.service_name, 'Venda durante atendimento'),
    ...
    IF v_appointment.service_name IS NOT NULL THEN
      v_description := v_description || ' ┬À Servi├ºo: ' || v_appointment.service_name;
    END IF;
    IF v_appointment.client_name IS NOT NULL THEN
      v_description := v_description || ' ┬À Cliente: ' || v_appointment.client_name;
    END IF;
  ...
  ELSIF EXISTS (
    SELECT 1 FROM public.commission_payments cp WHERE cp.appointment_id = p_appointment_id
  ) THEN
    ...
    ELSE
      ...
      ELSE
        INSERT INTO public.commission_payments (
          ...
          'pending',
          'Comiss├úo por ' || COALESCE(v_appointment.service_name, 'servi├ºo')
        )
        RETURNING id INTO v_commission_payment_id;

        IF v_commission_payment_id IS NULL THEN
          RAISE EXCEPTION 'Falha ao criar commission_payments (sem id retornado)';
        END IF;
  ...
  INSERT INTO public.appointment_completion_summaries (
    ...
    COALESCE(v_appointment.service_name, 'Servi├ºo'),
    v_service_profit,
    v_product_sales,
    v_product_profit,
    v_total_profit
  ...
  RETURN jsonb_build_object(
    ...
    'service_name', COALESCE(v_appointment.service_name, 'Servi├ºo'),
