-- Milestone 6: aplicar cashback automaticamente ao concluir atendimento
...
BEGIN
  ...
  -- Pacotes (sessões): consumir automaticamente ao concluir
  begin
    v_pkg_result := public.consume_package_session_for_appointment_v1(p_appointment_id);
  exception
    when undefined_function then
      v_pkg_result := null;
  end;

  -- Cashback: aplicar automaticamente ao concluir
  begin
    v_cashback_result := public.apply_cashback_for_appointment_v1(p_appointment_id);
  exception
    when undefined_function then
      v_cashback_result := null;
  end;
  ...
  RETURN jsonb_build_object(
    ...
    'package', v_pkg_result,
    'cashback', v_cashback_result
  );
END;
$$;
