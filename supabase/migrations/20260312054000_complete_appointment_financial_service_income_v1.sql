-- Patch: ao concluir atendimento (complete_appointment_with_sale), registrar receita do serviço em financial_transactions
-- e garantir transaction_date no fuso do app (America/Sao_Paulo), para refletir no "Saldo do Dia".

-- 1) Garantir idempotência para receita de serviço por agendamento
...
-- 2) Patch RPC: inserir receita do serviço (uma vez) e manter venda de produto com tx_date no fuso
...
BEGIN
  ...
  -- comissão + resumo (mantém lógica já existente nas versões mais recentes)
  -- A parte abaixo reaproveita o comportamento de inserir comissão e completion summary.
