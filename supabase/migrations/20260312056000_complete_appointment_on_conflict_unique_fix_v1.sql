-- Fix: erro "there is no unique or exclusion constraint matching the ON CONFLICT specification".
-- O ON CONFLICT só funciona se existir um índice UNIQUE/constraint compatível.
-- Aqui criamos os índices únicos (parciais) e ajustamos o ON CONFLICT para casar com o predicado do índice.

-- Receita de serviço (appointment_id + type + category, quando product_id IS NULL)
...
-- Venda de produto (appointment_id + product_id + type + category, quando product_id IS NOT NULL)
...
BEGIN
  ...
  IF v_service_price > 0 THEN
    ...
    ON CONFLICT (appointment_id, type, category) WHERE product_id IS NULL DO NOTHING;
  END IF;
  ...
  IF p_product_id IS NOT NULL THEN
    ...
    ON CONFLICT (appointment_id, product_id, type, category) WHERE product_id IS NOT NULL DO NOTHING;
  END IF;
