-- Fix: 42P10 "there is no unique or exclusion constraint matching the ON CONFLICT specification".
-- O ON CONFLICT com inferência de índice parcial exige que o WHERE do ON CONFLICT
-- implique o predicado do índice único. Os índices anteriores tinham predicados
-- com type/category constantes, mas o ON CONFLICT não incluía essas constantes,
-- então o Postgres não conseguia inferir o índice.
--
-- Solução: criar índices únicos parciais com predicados compatíveis com o ON CONFLICT
-- (apenas product_id IS NULL / IS NOT NULL) e manter o target do ON CONFLICT.

-- Serviço: única por (appointment_id,type,category) quando product_id IS NULL
...
-- Produto: única por (appointment_id,product_id,type,category) quando product_id IS NOT NULL
...
BEGIN
  ...
  IF v_service_price > 0 THEN
    ...
    ON CONFLICT (appointment_id, type, category)
      WHERE appointment_id IS NOT NULL AND product_id IS NULL
      DO NOTHING;
  END IF;
  ...
  IF p_product_id IS NOT NULL THEN
    ...
    ON CONFLICT (appointment_id, product_id, type, category)
