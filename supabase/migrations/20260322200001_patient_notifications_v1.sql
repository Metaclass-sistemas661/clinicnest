...
DROP POLICY IF EXISTS patient_notifications_update ON public.patient_notifications;
DROP POLICY IF EXISTS patient_notifications_insert_staff ON public.patient_notifications;
...
-- Habilitar Realtime (ignore if already added)
DO $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_notify_patient_certificate ON public.medical_certificates;
...
DROP TRIGGER IF EXISTS trg_notify_patient_prescription ON public.prescriptions;
CREATE TRIGGER trg_notify_patient_prescription
  AFTER INSERT ON public.prescriptions
  FOR EACH ROW EXECUTE FUNCTION public.notify_patient();

DROP TRIGGER IF EXISTS trg_notify_patient_exam ON public.exam_results;
