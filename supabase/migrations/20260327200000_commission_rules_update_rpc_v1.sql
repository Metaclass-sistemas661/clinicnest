-- Migration: Sub-fase 31A — Parte 3: Atualizar RPC complete_appointment_with_sale
-- Usa get_applicable_commission_rule em vez de busca simples
...
DECLARE
  ...
  -- Nova estrutura para regras de comissão
...
BEGIN
  ...
  -- ========== NOVA LÓGICA DE COMISSÃO COM REGRAS AVANÇADAS ==========
  IF v_professional_user_id IS NULL THEN
    v_commission_reason := 'missing_professional_user_id';
  ELSE
    -- Buscar regra de comissão aplicável usando a nova função
    ...
    FROM public.get_applicable_commission_rule(
      ...
      NULL -- procedure_code (futuro)
    );

    IF v_commission_rule IS NULL OR v_commission_rule.rule_id IS NULL THEN
      -- Fallback: tentar buscar na tabela antiga professional_commissions
      SELECT 
        cr.id AS rule_id,
      ...
      LEFT JOIN public.commission_rules cr ON cr.professional_id = pc.user_id 
                                           AND cr.tenant_id = pc.tenant_id
                                           AND cr.rule_type = 'default'
    ...
    ELSE
      -- Calcular faturamento mensal do profissional (para comissões escalonadas)
      ...
      -- Override se appointment tem commission_amount definido
