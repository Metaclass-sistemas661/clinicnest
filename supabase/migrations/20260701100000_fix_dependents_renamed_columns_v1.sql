-- ============================================================================
-- FIX: Update functions and RLS after patient_dependents column rename
-- parent_client_id -> parent_patient_id
-- dependent_client_id -> dependent_patient_id
-- ============================================================================
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    pd.dependent_patient_id as dependent_id,
...
-- 4) Fix remove_patient_dependent
...
-- 5) Fix patient_create_appointment (dependent validation part)
DROP FUNCTION IF EXISTS public.patient_create_appointment(uuid, uuid, timestamptz, uuid);
...
BEGIN
  ...
  IF p_for_dependent_id IS NOT NULL THEN
    SELECT EXISTS(
      SELECT 1 FROM public.patient_dependents pd
      WHERE pd.parent_patient_id = v_client_id
      AND pd.dependent_patient_id = p_for_dependent_id
      AND pd.is_active = true
  ...
  WHERE (
    a.patient_id = v_client_id 
    OR a.patient_id IN (
      SELECT pd.dependent_patient_id 
      FROM public.patient_dependents pd 
      WHERE pd.parent_patient_id = (
  ...
  SELECT s.name INTO v_service_name FROM public.procedures s WHERE s.id = p_procedure_id;
  SELECT p.full_name INTO v_professional_name FROM public.profiles p WHERE p.id = p_professional_id;

  INSERT INTO public.appointments (
    tenant_id, patient_id, procedure_id, professional_id, scheduled_at, status, source, notes
  ) VALUES (
    v_tenant_id, v_client_id, p_procedure_id, p_professional_id, p_scheduled_at, 'pending', 'patient_portal',
