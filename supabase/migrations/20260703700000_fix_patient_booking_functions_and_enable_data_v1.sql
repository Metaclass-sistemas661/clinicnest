-- ============================================================================
-- Migration: 20260703700000_fix_patient_booking_functions_and_enable_data_v1
--
-- Problemas:
--   1. Coluna service_type nunca renomeada para procedure_type
--   2. get_patient_bookable_services() → usa public.services (renomeado)
--   3. get_available_slots_for_patient() → usa public.services%ROWTYPE
--   4. patient_create_appointment() → usa client_id, service_id (renomeados)
--   5. submit_appointment_rating() → usa a.client_id (renomeado)
--   6. patient_bookable = false em TODOS os procedimentos e profissionais
--
-- Solução:
--   - Renomear coluna service_type → procedure_type
--   - Recriar as 4 funções com nomes de tabela/coluna corretos
--   - Ativar patient_bookable para procedimentos e profissionais ativos
-- ============================================================================

-- ============================================================================
-- 0. Renomear coluna service_type → procedure_type (esquecida na 20260330400000)
-- ============================================================================
DO $$ BEGIN
  IF EXISTS (
...
DECLARE
  v_con text;
BEGIN
  -- Encontrar e dropar constraint antiga que referencia service_type
...
-- ============================================================================
-- 1. get_patient_bookable_services — procedures.procedure_type AS category
-- ============================================================================
...
-- ============================================================================
-- 2. get_available_slots_for_patient — services%ROWTYPE → procedures%ROWTYPE
-- ============================================================================
...
-- ============================================================================
-- 3. patient_create_appointment — client_id → patient_id, service_id → procedure_id
-- ============================================================================
...
BEGIN
  ...
  IF p_for_dependent_id IS NOT NULL THEN
    -- Verify the dependent belongs to this patient
    IF NOT EXISTS (
  ...
  -- Check working hours (use helper if exists, otherwise skip)
  BEGIN
  ...
  EXCEPTION WHEN undefined_function THEN
    -- Function doesn't exist, skip working hours check
    NULL;
  END;
...
-- Also keep old signature working if called without dependent
-- (the frontend might call with p_service_id name)
DROP FUNCTION IF EXISTS public.patient_create_appointment(uuid, uuid, timestamptz);

-- ============================================================================
-- 4. submit_appointment_rating — a.client_id → a.patient_id
-- ============================================================================
...
-- ============================================================================
-- 5. ATIVAR patient_bookable para procedimentos e profissionais ativos
-- ============================================================================

-- Ativar todos os procedimentos ativos como bookable pelo paciente
UPDATE public.procedures
SET patient_bookable = true
WHERE is_active = true AND patient_bookable = false;

-- Ativar todos os profissionais ativos como bookable pelo paciente
UPDATE public.profiles
SET patient_bookable = true
WHERE patient_bookable = false;

-- Mudar default para true em novos registros
