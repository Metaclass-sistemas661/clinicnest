-- ============================================================================
-- SECURITY: Hash criptográfico de consentimento + server-side timestamp
--
-- Problemas corrigidos:
--   1. Assinatura era apenas foto + IP (não verificável criptograficamente)
--   2. Timestamp vinha do cliente (manipulável)
--
-- Solução:
--   - Coluna document_hash (SHA-256 do HTML do termo no momento da assinatura)
--   - signed_at usa now() (server-side, não manipulável)
--   - Atualizar sign_consent e sign_consent_v2 para computar hash
-- ============================================================================

-- 1. Adicionar coluna document_hash
ALTER TABLE public.patient_consents
  ADD COLUMN IF NOT EXISTS document_hash TEXT;

COMMENT ON COLUMN public.patient_consents.document_hash IS 'SHA-256 hash of template_snapshot_html at signing time for tamper detection';

-- 2. Atualizar sign_consent para incluir hash
...
-- 3. Atualizar sign_consent_v2 para incluir hash
